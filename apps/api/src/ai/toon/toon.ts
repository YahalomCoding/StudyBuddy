...
var ToonDecodeError = class extends SyntaxError {
  constructor(message, context) {
    ...
    super(
      prefix + message,
      context?.cause !== void 0 ? { cause: context.cause } : void 0
    );
...
function parseArrayHeaderLine(content, defaultDelimiter, strict = false) {
  ...
  if (trimmedToken.startsWith('"')) {
    ...
    const keyEndIndex =
      content.length - trimmedToken.length + closingQuoteIndex + 1;
  ...
  if (
    braceStart !== -1 &&
    braceStart < findUnquotedChar(content, ":", bracketEnd)
  ) {
    ...
    const foundBraceEnd = findUnquotedChar(content, "}", braceStart);
  ...
  if (braceStart !== -1 && braceStart < colonIndex) {
    ...
    if (foundBraceEnd !== -1 && foundBraceEnd < colonIndex) {
      ...
      const mismatchedDelimiter = findUnquotedMismatchedDelimiter(
        fieldsContent,
        delimiter
      );
      if (mismatchedDelimiter !== void 0) {
        if (strict)
          throw new SyntaxError(
            `Header delimiter mismatch: bracket declares "${formatDelimiter(delimiter)}" but fields segment contains unquoted "${formatDelimiter(mismatchedDelimiter)}"`
          );
        return;
      }
      fields = parseDelimitedValues(fieldsContent, delimiter).map((field) =>
...
function parseUnquotedKey(content, start) {
  ...
  while (parsePosition < content.length && content[parsePosition] !== ":")
    parsePosition++;
...
function isArrayHeaderContent(content) {
  return (
    content.trim().startsWith("[") && findUnquotedChar(content, ":") !== -1
  );
}
...
function parseLineIncremental(raw, state, indentSize, strict) {
  ...
  if (strict) {
    ...
    while (
      ...
      (raw[whitespaceEndIndex] === " " || raw[whitespaceEndIndex] === "	")
    )
      whitespaceEndIndex++;
    if (raw.slice(0, whitespaceEndIndex).includes("	"))
...
function* decodeStreamSync$1(source, options) {
  ...
  const cursor = new StreamingLineCursor(
    parseLinesSync(
      source,
      resolvedOptions.indent,
      resolvedOptions.strict,
      scanState
    ),
    scanState
  );
  ...
  if (isArrayHeaderContent(first.content)) {
    const headerInfo = withLine(first, () =>
    ...
    if (headerInfo) {
      cursor.advanceSync();
      yield* decodeArrayFromHeaderSync(
        headerInfo.header,
  ...
  const rootSeenKeys = resolvedOptions.strict
    ? /* @__PURE__ */ new Set()
    : void 0;
  yield { type: "startObject" };
...
function* decodeKeyValueSync(line, cursor, baseDepth, options, seenKeys) {
  ...
  const arrayHeader = withLine(line, () =>
    parseArrayHeaderLine(content, DEFAULT_DELIMITER, options.strict)
  );
  ...
  const { key, isQuoted, end } = withLine(line, () =>
    parseKeyToken(content, 0)
  );
...
function* decodeArrayFromHeaderSync(
  ...
  if (inlineValues) {
    yield* decodeInlinePrimitiveArraySync(
      header,
      inlineValues,
      options,
      headerLine
...
function* decodeInlinePrimitiveArraySync(
  ...
  const values = withLine(headerLine, () =>
    parseDelimitedValues(inlineValues, header.delimiter)
  );
  const primitives = withLine(headerLine, () =>
...
function* decodeTabularArraySync(
  ...
  while (!cursor.atEndSync() && rowCount < header.length) {
    ...
    if (line.depth === rowDepth) {
      ...
      lastRowLine = line;
      cursor.advanceSync();
      const values = withLine(line, () =>
        parseDelimitedValues(line.content, header.delimiter)
      );
      assertExpectedCount(
        values.length,
...
function* decodeListItemSync(cursor, baseDepth, options) {
  ...
  if (isArrayHeaderContent(afterHyphen)) {
    const arrayHeader = withLine(itemLine, () =>
      parseArrayHeaderLine(afterHyphen, DEFAULT_DELIMITER, options.strict)
    );
    if (arrayHeader) {
      yield* decodeArrayFromHeaderSync(
        arrayHeader.header,
  ...
  const headerInfo = withLine(itemLine, () =>
    parseArrayHeaderLine(afterHyphen, DEFAULT_DELIMITER, options.strict)
  );
  if (
    headerInfo &&
    headerInfo.header.key !== void 0 &&
    headerInfo.header.fields !== void 0
  ) {
    const header = headerInfo.header;
    const seenKeys = options.strict
      ? /* @__PURE__ */ new Set([header.key])
      : void 0;
    yield { type: "startObject" };
    yield {
      type: "key",
    ...
    while (!cursor.atEndSync()) {
      ...
      if (
        nextLine.depth === followDepth &&
        !nextLine.content.startsWith("- ")
      ) {
        cursor.advanceSync();
        yield* decodeKeyValueSync(
          nextLine,
          cursor,
          followDepth,
          options,
...
async function* decodeStream$1(source, options) {
  ...
  if (Symbol.asyncIterator in source) {
    ...
    if (isArrayHeaderContent(first.content)) {
      ...
      if (headerInfo) {
        await cursor.advance();
        yield* decodeArrayFromHeaderAsync(
          headerInfo.header,
          headerInfo.inlineValues,
          cursor,
          0,
...
async function* decodeTabularArrayAsync(
  ...
  if (options.strict)
    validateNoExtraTabularRows(await cursor.peek(), rowDepth, header);
}
async function* decodeListArrayAsync(
  ...
  if (options.strict)
    validateNoExtraListItems(await cursor.peek(), itemDepth, header.length);
}
...
function getOwnProperty(target, key) {
  return Object.hasOwn(target, key) ? target[key] : void 0;
}
/**
...
function expandPathsSafe(value, strict) {
  ...
  if (isJsonObject(value)) {
    ...
    for (const [key, keyValue] of Object.entries(value)) {
      ...
      if (key.includes(".") && !isQuoted) {
        ...
        if (segments.every((seg) => isIdentifierSegment(seg))) {
          insertPathSafe(
            expandedObject,
            segments,
            expandPathsSafe(keyValue, strict),
            strict
          );
          continue;
        }
      ...
      } else setOwnProperty(expandedObject, key, expandedValue);
    }
...
function insertPathSafe(target, segments, value, strict) {
  ...
  if (destinationValue === void 0) setOwnProperty(currentNode, lastSeg, value);
...
function* encodeArrayLines(key, value, depth, options) {
  ...
  if (isArrayOfPrimitives(value)) {
    yield indentedLine(
      depth,
      encodeInlineArrayLine(value, options.delimiter, key),
      options.indent
    );
    return;
  }
 ...
 * ```
 */
function encode(input: unknown, options?: unknown) {
...
function decodeFromLines(lines, options) {
  ...
  const decodedValue = buildValueFromEvents(
    decodeStreamSync$1(lines, {
