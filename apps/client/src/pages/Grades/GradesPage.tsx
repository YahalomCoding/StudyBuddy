...
} from "../../components/UpcomingEvents/mockData";
  ...
  const courseSummaries = useMemo<CourseGradeSummary[]>(() => {
    return courses
      .map((course) => {
        ...
        const studentSemesterCourse = semesterCourse
          ? studentSemesterCourses.find(
              (item) => item.semesterCourseId === semesterCourse.id
            )
          : undefined;

        const courseExams = studentSemesterCourse
          ? exams.filter(
              (exam) =>
                exam.studentSemesterCourseId === studentSemesterCourse.id
            )
          : [];

        const courseAssignments = studentSemesterCourse
          ? assignments.filter(
              (assignment) =>
                assignment.studentSemesterCourseId === studentSemesterCourse.id
            )
          : [];
        ...
        const examGrade =
          courseExams.length > 0
            ? roundGrade(
                courseExams.reduce((sum, exam) => sum + exam.grade, 0) /
                  courseExams.length
              )
            : null;

        const assignmentGrades = courseAssignments.filter(
          (assignment): assignment is typeof assignment & { grade: number } =>
        ...
        const finalGrade =
          examGrade !== null && assignmentGrade !== null
            ? roundGrade(examGrade * 0.6 + assignmentGrade * 0.4)
